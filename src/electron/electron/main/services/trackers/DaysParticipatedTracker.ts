...
export class DaysParticipatedTracker implements Tracker {
  ...
  private async updateDaysParticipated() {
    ...
    if (!studyConfig.trackers.experienceSamplingTracker.enabled) {
      LOG.info(
        'experience sampling tracker is disabled, counting "days participated" based on user input only.'
      );
    ...
    daysUsage.forEach((day) => {
      if (daysSamplingResponses.has(day)) {
        count++;
      }
    });
  ...
  private async startCountingJob(): Promise<void> {
    ...
    this.countParticipatedDaysJob = schedule.scheduleJob(
      '0 * * * *',
      this.updateDaysParticipated.bind(this)
    );
  }
